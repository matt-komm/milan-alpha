...
{    
    using namespace milan;
    Parameter signalStrength("signalStrength",1.0);
    ...
    Parameter backgroundYield("backgroundYield",1.0);
    
    Histogram backgroundTemplate1({Binning(bins,-1,1)});
    HistogramFunction backgroundPrediction = HistogramFunction(backgroundTemplate1.ref())*backgroundYield;
    ...
    for (unsigned int i = 0; i < bins; ++i)
    {
        ...
        backgroundTemplate1.setContent(i,3.0+i*0.25);
        backgroundTemplate1.setError2(i,1.5+std::sqrt(i*0.25));
